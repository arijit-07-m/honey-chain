...
export default function AlertsPage() {
  ...
  useEffect(() => {
    if (!token) return;
    getAlerts(token).then(a => { setAlerts(a); setLoading(false); }).catch(() => setLoading(false));
  }, [token]);
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-3">
        {alerts.length === 0 && (
          <div className="card text-center py-12">
            ...
            <p className="text-gray-500">No alerts. All hives are healthy.</p>
          </div>
        )}
        {alerts.map(alert => (
          <div key={alert.id} className={`card border-l-4 ${alert.resolved ? 'border-gray-300' : 'border-orange-400'} ${alert.resolved ? '' : 'bg-orange-50'}`}>
            <div className="flex items-start gap-3">
              <AlertTriangle className={`w-5 h-5 mt-0.5 flex-shrink-0 ${alert.resolved ? 'text-gray-400' : 'text-orange-500'}`} />
              <div className="flex-1">
                <p className="font-medium text-gray-900">{alert.message}</p>
                <p className="text-xs text-gray-400 mt-1">{new Date(alert.created_at).toLocaleString()}</p>
                <button onClick={() => router.push(`/farmer/hives/${alert.hive_id}`)}
                  className="text-sm text-honey-600 font-medium mt-1 hover:underline">View Hive →</button>
              </div>
